...
- Автоматическое скачивание PDF-отчёта по завершении скана (для публикации в GitLab CI/CD artifacts)
...
report:
  enabled: true              # Скачивать PDF-отчёт после завершения скана
  output_path: "./report.pdf" # Куда сохранить PDF
  language: "en"             # Язык отчёта: "en" или "ru"
```
...
Секция необязательна — при отсутствии применяются значения по умолчанию.

| Параметр      | Тип    | По умолчанию   | Описание                                   |
| ------------- | ------ | -------------- | ------------------------------------------ |
| `enabled`     | bool   | `true`         | Скачивать PDF-отчёт после завершения скана |
| `output_path` | string | `./report.pdf` | Путь сохранения PDF                        |
| `language`    | string | `en`           | Язык отчёта: `en` или `ru`                 |

#### Когда отчёт скачивается

| Ситуация                                           | Скачивается? |
| -------------------------------------------------- | ------------ |
| Успешное завершение скана (все задачи выполнены)   | ✔            |
| Превышение `max_runtime_hours`                     | ✔            |
| Получение `SIGINT` / `SIGTERM` (graceful shutdown) | ✔ (попытка)  |
| Потеря авторизации в целевом приложении            | ✘            |

При ошибке скачивания (сетевой сбой, недоступность API) скрипт логирует ошибку,
но не меняет итоговый exit code — поведение скана приоритетнее.
...
По завершении скана скрипт скачивает PDF-отчёт по пути из `report.output_path`.
Этот файл можно опубликовать как артефакт пайплайна.
...
Если путь в `report.output_path` отличается от корня репозитория, укажите его в
`artifacts.paths` — он должен совпадать с `output_path` из конфига.
...
5. **Скачивание PDF-отчёта** (если `report.enabled: true` и авторизация не была потеряна)
